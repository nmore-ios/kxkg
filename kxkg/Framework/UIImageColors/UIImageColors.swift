...
extension UIImage {
    ...
    public func getColors(scaleDownSize: CGSize) -> UIImageColors {
        ...
        let bytesPerRow: Int = Int(width) * bytesPerPixel
        ...
        let raw = malloc(Int(bytesPerRow) * height)
        let bitmapInfo = CGBitmapInfo(rawValue: CGImageAlphaInfo.PremultipliedFirst.rawValue)
        let ctx = CGBitmapContextCreate(raw, width, height, Int(bitsPerComponent), Int(bytesPerRow), colorSpace, bitmapInfo.rawValue)
        ...
        let leftEdgeColors = NSCountedSet(capacity: Int(height))
        let imageColors = NSCountedSet(capacity:Int(width * height))
