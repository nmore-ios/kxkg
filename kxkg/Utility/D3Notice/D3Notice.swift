...
extension UIViewController {
    //suc,3秒自动消失
    func showNoticeSuc(text: String,viewHieght:CGFloat) {
        D3Notice.showNoticeWithText(NoticeType.success, text: text,time: D3Notice.longTime, autoClear: true,viewHieght:viewHieght)
    }
    
    func showNoticeSuc(text: String,time:NSTimeInterval, autoClear: Bool,viewHieght:CGFloat) {
        D3Notice.showNoticeWithText(NoticeType.success, text: text,time: time, autoClear: autoClear,viewHieght:viewHieght)
    }
    
    //err
    func showNoticeErr(text: String,viewHieght:CGFloat) {
        D3Notice.showNoticeWithText(NoticeType.error, text: text,time: D3Notice.longTime, autoClear: true,viewHieght:viewHieght)
    }
    func showNoticeErr(text: String,time:NSTimeInterval, autoClear: Bool,viewHieght:CGFloat) {
        D3Notice.showNoticeWithText(NoticeType.error, text: text, time:time,autoClear: autoClear,viewHieght:viewHieght)
    }
    
    //info
    func showNoticeInfo(text: String,viewHieght:CGFloat) {
        D3Notice.showNoticeWithText(NoticeType.info, text: text, time: D3Notice.longTime,autoClear: true,viewHieght:viewHieght)
    }
    func showNoticeInfo(text: String,time:NSTimeInterval, autoClear: Bool,viewHieght:CGFloat) {
        D3Notice.showNoticeWithText(NoticeType.info, text: text, time: time,autoClear: autoClear,viewHieght:viewHieght)
    }
...
class D3Notice: NSObject {
    ...
    //有勾、叉和警告
    static func showNoticeWithText(type: NoticeType,text: String,time:NSTimeInterval,autoClear: Bool,viewHieght:CGFloat?) {
        clear()
        let frame:CGRect;
        if viewHieght != nil{
            frame = CGRectMake(0, 0, 90,viewHieght!)
        }else{
        ...
        let label = UILabel()
        if viewHieght != nil{
            label.frame=CGRectMake(5,60,80,32)
        }else{
            label.frame=CGRectMake(5,60,80,16)
        }
